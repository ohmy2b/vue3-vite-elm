    ...
    export default {
      ...
      data(){
            ...
            const initData=async()=>{
                state.serviceData = await getService();
                Object.keys(state.serviceData).forEach(item => {
                    let avoidRepeat = false;
                    state.questionTitle.forEach((insertItem) => {
                        //防止重复的数据，后台返回的数据有些是重复的
                        if (insertItem == state.serviceData[item]) {
                            avoidRepeat = true;
                        }
                    })
                    //将标题和内容分别放进数组中
                    if (item.indexOf('Caption') !== -1 && !avoidRepeat) {
                            state.questionTitle.push(state.serviceData[item]);
                    }else if(!avoidRepeat){
                        state.questionDetail.push(state.serviceData[item]);
                    }
                })
            }
            //保存问题详情
            const toQuestionDetail=(title, index)=>{
                useMutation.SAVE_QUESTION({title, detail: state.questionDetail[index]});
                router.push('/service/questionDetail');
            }
            return {...toRefs(state),toQuestionDetail,initData}
        },
