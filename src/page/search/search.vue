...
export default {
    ...
    onMounted(()=>{
        state.geohash = state.$route.params.geohash;
        //获取搜索历史记录
        ...
        const searchTarget=async(historyValue)=>{
            if (historyValue) {
                state.searchValue = historyValue;
            ...
            //获取搜索结果
            state.restaurantList = await searchRestaurant(state.geohash, state.searchValue);
            state.emptyResult = !state.restaurantList.length;
            /**
        ...
        const checkInput=()=>{
            if (state.searchValue === '') {
                state.showHistory = true; //显示历史记录
                state.restaurantList = []; //清空搜索结果
                state.emptyResult = false; //隐藏搜索为空提示
            } 
        }
        //点击删除按钮，删除当前历史记录
        ...
        return {...toRefs(state),clearAllHistory,deleteHistory,checkInput,searchTarget}
