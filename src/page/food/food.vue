...
export default {
  ...
  data(mixins) {
    ...
    const state=reactive({
      ...
      latitude=store.state.latitude,longitude=store.state.longitude
    })
  onMounted(()=>{initData()})
    const initData=async()=>{
      ...
      //获取category分类左侧数据
      state.category = await foodCategory(state.latitude, state.longitude);
      //初始化时定位当前category分类左侧默认选择项，在右侧展示出其sub_categories列表
      state.category.forEach(item => {
        if (state.restaurant_category_id == item.id) {
          state.categoryDetail = item.sub_categories;
        }
      });
      //获取筛选列表的配送方式
      state.Delivery = await foodDelivery(state.latitude, state.longitude);
      //获取筛选列表的商铺活动
      state.Activity = await foodActivity(state.latitude, state.longitude);
      //记录support_ids的状态，默认不选中，点击状态取反，status为true时为选中状态
      state.Activity.forEach((item, index) => {
        state.support_ids[index] = { status: false, id: item.id };
      });
    }
    ...
    const selectCategoryName=(id, index)=>{
      ...
      if (index === 0) {
        state.restaurant_category_ids = null;
        state.sortBy = "";
        //不是第一个选项时，右侧展示其子级sub_categories的列表
      } else {
        state.restaurant_category_id = id;
        state.categoryDetail = state.category[index].sub_categories;
      }
    },
    //选中Category右侧列表的某个选项时，进行筛选，重新获取数据并渲染
    const getCategoryIds=(id, name)=>{
	  //console.log(id, name)
      state.restaurant_category_ids = id;
      state.sortBy = "";
      state.foodTitle = state.headTitle = name;
    }
    ...
    const sortList=(event)=> {
      ...
      state.sortByType = node.getAttribute("data");
      state.sortBy = "";
    }
    //筛选选项中的配送方式选择
    const selectDeliveryMode=(id)=> {
      ...
      } else if (state.delivery_mode == id) {
        state.filterNum--;
    ...
    const selectSupportIds=(index, id)=>{
      //数组替换新的值
      state.support_ids.splice(index, 1, {
        status: !state.support_ids[index].status,
        id
      });
      //重新计算filterNum的个数
      state.filterNum = state.delivery_mode == null ? 0 : 1;
      state.support_ids.forEach(item => {
        if (item.status) {
          state.filterNum++;
        }
      });
    ...
    const clearSelect=()=>{
      state.support_ids.map(item => (item.status = false));
      state.filterNum = 0;
      state.delivery_mode = null;
    }
    ...
    return {...toRefs(state),chooseType,clearSelect,confirmSelectFun,selectCategoryName,selectDeliveryMode,sortList,getCategoryIds,selectSupportIds}
  }
}
</script>
