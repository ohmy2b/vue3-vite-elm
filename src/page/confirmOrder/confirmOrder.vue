...
<script>
    import {reactive,toRefs,onMounted,onCreated}from 'vue'
    ...
    export default {
        ...
        setup(){
            ...
            let useMutation=store._mutaitons
        ...
        onMounted(()=>{
            ...
            if (!(store.state.userInfo && store.state.userInfo.user_id)) {
                ...
                // state.alertText = '您还没有登录';
            }
        })
        watch(()=>store.state.userInfo,(val)=>{initAddress()})
        remarklist=computed(()=>{
                let str = new String;
                ...
                //是否有自定义备注，分开处理
                if (store.state.inputText) {
                    return str + store.state.inputText;
                }else{
        ...
        //     ]),
            ...
            const initData=async()=>{
                ...
                useMutation.SAVE_CART_ID_SIG[0]({cart_id: state.checkoutData.cart.id, sig:  state.checkoutData.sig})
                initAddress();
            ...
            const initAddress=async()=>{
                if (store.state.userInfo && store.state.userInfo.user_id) {
                    ...
                    if (addressRes instanceof Array && addressRes.length) {
                        useMutation.CHOOSE_ADDRESS[0]({address: addressRes[0], index: 0});
                    }
                }
            }
            //显示付款方式
            ...
            const confrimOrder=async()=>{
                ...
                if (!(store.state.userInfo && store.state.userInfo.user_id)) {
                    ...
                    return
                    //未选择地址则提示
                }else if(!store.state.choosedAddress){
                    state.showAlert = true;
                    state.alertText = '请添加一个收货地址';
                    return
                }
                //保存订单
                state.SAVE_ORDER_PARAM({
                    user_id: store.state.userInfo.user_id,
                    cart_id: state.checkoutData.cart.id,
                    address_id: store.state.choosedAddress.id,
                    description: state.remarklist,
                    entities: state.checkoutData.cart.groups,
                    geohash: state.geohash,
                    sig: state.checkoutData.sig,
                });
                //发送订单信息
                let orderRes = await placeOrders(store.state.userInfo.user_id, state.checkoutData.cart.id, store.state.choosedAddress.id, state.remarklist, state.checkoutData.cart.groups, state.geohash, state.checkoutData.sig);
                //第一次下单的手机号需要进行验证，否则直接下单成功
                if (orderRes.need_validation) {
                    useMutation.NEED_VALIDATION[0](orderRes);
        ...
        return {...toRefs(state),confrimOrder,iconColor,choosePayWay,showPayWayFun,remarklist}
        // watch: {
