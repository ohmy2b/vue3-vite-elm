      ...
      setup(){
            ...
            const addAddress=async()=>{
                if (!(store.state.userInfo && store.state.userInfo.user_id)) {
                    ...
                    state.showAlert = true;
                    state.alertText = '请输入姓名'
                }else if(!state.phone){
                    state.showAlert = true;
                    state.alertText = '请输入电话号码'
                }else if(!state.searchAddress){
                    state.showAlert = true;
                    state.alertText = '请选择地址'
                }else if(!state.address_detail){
                    state.showAlert = true;
                    state.alertText = '请输入详细地址'
                }
                if (state.tag == '家') {
                    state.tag_type = 2;
                }else if(state.tag == '学校'){
                    state.tag_type = 3;
                }else if(state.tag == '公司'){
                    state.tag_type = 4;
                }
                let res = await postAddAddress(store.state.userInfo.user_id, store.state.searchAddress.name, state.address_detail, store.state.geohash, state.name, state.phone, state.anntherPhoneNumber, 0, state.sex, state.tag, state.tag_type);
                //保存成功返沪上一页，否则弹出提示框
                if (res.message) {
                    state.showAlert = true;
                    state.alertText = res.message;
                }else {
                    useMutation.CONFIRM_ADDRESS[0](1);
                    router.go(-1);
                }
            }
            return {...toRefs(state),addAddress,chooseSex}
        // components: {headTop,alertTip},
        // computed: {
        //     ...mapState([
        ...
        //     ]),
            ...
            //         state.tag_type = 4;
            //     }
            //     let res = await postAddAddress(state.userInfo.user_id, state.searchAddress.name, state.address_detail, state.geohash, state.name, state.phone, state.anntherPhoneNumber, 0, state.sex, state.tag, state.tag_type);
            //     //保存成功返沪上一页，否则弹出提示框
            //     if (res.message) {
            //         state.showAlert = true;
            //         state.alertText = res.message;
