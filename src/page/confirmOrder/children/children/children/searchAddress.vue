      ...
      setup(){
            ...
            const searchPlace=async()=>{
                if (state.searchValue) {
                    state.searchData = await searchNearby(state.searchValue);
                }
            }
            //选择搜素结果
            const choooedAddress=(item)=>{
                useMutation.CHOOSE_SEARCH_ADDRESS[0](item);
                router.go(-1);
            }
            return {...toRefs(state),choooedAddress,searchPlace}
        },
        // methods: {
        //     ...mapMutations([
