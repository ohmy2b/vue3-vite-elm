...
<script>
    import {reactive,toRefs,onMounted,created}from 'vue'
      ...
      setup(){
        ...
        onMounted(()=>{initDate()})
            const initDate=async()=>{
                state.remarkList = await getRemark(state.id, state.sig);
                state.showLoading = false;
            }
            //选择备注
            const chooseRemark=(index, remarkIndex, text)=>{
                state.remarkText[index] = [remarkIndex, text];
                state.remarkText = Object.assign({}, state.remarkText);
            },
            //确认选择
            const confirmRemark=()=>{
                useMutation.CONFIRM_REMARK[0]({remarkText: state.remarkText, inputText: state.inputText});
                router.go(-1);
            }
            return {...toRefs(state),confirmRemark,chooseRemark,initDate}
        // methods: {
        //     ...mapMutations([
        //         'CONFIRM_REMARK'
        //     ]),
