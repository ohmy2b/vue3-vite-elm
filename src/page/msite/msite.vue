...
export default {
	...
	setup(){
		const route=useRoute(),store=useStore(),useMutations=store._mutations
    ...
    onActivated(async()=>{
		...
		//保存geohash 到vuex
		useMutations.SAVE_GEOHASH(state.geohash);
    	//获取位置信息
    	let res = await msiteAddress(state.geohash);
    	state.msiteTitle = res.name;
    	// 记录当前经度纬度
    	useMutations.RECORD_ADDRESS(res);

    	state.hasGetData = true;
    })
    onMounted(()=>{
        //获取导航食品类型列表
       	msiteFoodTypes(state.geohash).then(res => {
