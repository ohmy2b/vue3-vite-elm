...
<script>
    ...
    export default {
      ...
      setup(){
            ...
            const status=computed(()=>(/^\d+$/gi.test(this.exchangeCode)) && (/^\w{4}$/gi.test(this.codeNumber)))
            ...
            const exchange=async()=>{
                if (state.status) {
                    let res = await exChangeHongbao(state.userInfo.user_id, state.exchangeCode, state.codeNumber);
                    //不成功则弹出提示框
                    if (res.message) {
                        getCaptchaCode();
            ...
            return {...toRefs(state),status,exchange,getCaptchaCode}
        }
        // mounted(){
        //     this.getCaptchaCode();
        // },
