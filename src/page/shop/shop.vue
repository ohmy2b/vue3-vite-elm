...
<script>
    import {reactive,toRefs,onCreated,onMounted}from 'vue'
    ...
    export default {
        ...
        beforeUnmounted(()=>{
            // state.foodScroll.removeEventListener('scroll', )
        })
        promotionInfo=computed(()=>state.shopDetailData.promotion_info || '欢迎光临，用餐高峰期请提前下单，谢谢。')
        ...
        shopCart=computed(()=>{
                return {...this.cartList[state.shopId]};
            })
            //购物车中总共商品的数量
        totalNum=computed(()=>{
                let num = 0;
        ...
        const initData=async()=>{
                ...
                //获取商铺信息
                state.shopDetailData = await shopDetails(state.shopId, this.latitude, this.longitude);
                //获取商铺食品列表
            ...
            const getFoodListHeight=()=>{
                const listContainer = refs.menuFoodList;
                if (listContainer) {
            ...
            const listenScroll=(element)=>{
                ...
                const wrapMenuHeight = refs.wrapperMenu.clientHeight;
                state.foodScroll.on('scroll', (pos) => {
                    ...
                    state.shopListTop.forEach((item, index) => {
                        if (state.menuIndexChange && Math.abs(Math.round(pos.y)) >= item) {
                            ...
                            const menuList=refs.wrapperMenu.querySelectorAll('.activity_menu');
                            const el = menuList[0];
                        ...
                        Object.keys(this.shopCart[item.foods[0].category_id]).forEach(itemid => {
                            Object.keys(this.shopCart[item.foods[0].category_id][itemid]).forEach(foodid => {
                                ...
                                if (item.type == 1) {
                                    ...
                                    if (foodItem.num > 0) {
                                        state.cartFoodList[cartFoodNum] = {};
                                        state.cartFoodList[cartFoodNum].category_id = item.foods[0].category_id;
                                        state.cartFoodList[cartFoodNum].item_id = itemid;
                                        state.cartFoodList[cartFoodNum].food_id = foodid;
                                        state.cartFoodList[cartFoodNum].num = foodItem.num;
                                        state.cartFoodList[cartFoodNum].price = foodItem.price;
                                        state.cartFoodList[cartFoodNum].name = foodItem.name;
                                        state.cartFoodList[cartFoodNum].specs = foodItem.specs;
                                        cartFoodNum ++;
                                    }
                                }
                            })
            ...
            const listenInCart=()=>{
                if (!state.receiveInCart) {
                    ...
                    refs.cartContainer.addEventListener('animationend', () => {
                        state.receiveInCart = false;
                    })
                    refs.cartContainer.addEventListener('webkitAnimationEnd', () => {
                        state.receiveInCart = false;
                    })
                }
            }
            ...
            const loaderMoreRating=async()=>{
                if (state.preventRepeatRequest) {
                    return
                }
                state.loadRatings = true;
                state.preventRepeatRequest = true;
                state.ratingOffset += 10;
                let ratingDate = await getRatingList(state.shopId, state.ratingOffset, state.ratingTagName);
                state.ratingList = [...state.ratingList,...ratingDate];
                state.loadRatings = false;
                if (ratingDate.length >= 10) {
                    state.preventRepeatRequest = false;
                }
            }
            //隐藏动画
            const hideLoading=()=>{
            ...
            //多规格商品加入购物车
            const addSpecs=(category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock)=>{
            ...
            const beforeEnter=(el)=>{
                el.style.transform = `translate3d(0,${37 + state.elBottom - state.windowHeight}px,0)`;
                el.children[0].style.transform = `translate3d(${state.elLeft - 30}px,0,0)`;
        ...
        //         return state.shopDetailData.promotion_info || '欢迎光临，用餐高峰期请提前下单，谢谢。'
        //     },
        //     //配送费
        ...
        //     ]),
            ...
            //     //评论Tag列表
            //     state.ratingTagsList = await ratingTags(state.shopId);
            //     this.RECORD_SHOPDETAIL(state.shopDetailData)
            //     //隐藏加载动画
            //     this.hideLoading();
            // },
            // //获取食品列表的高度，存入shopListTop
            ...
            //     this.ADD_CART({shopid: state.shopId, category_id, item_id, food_id, name, price, specs});
            // },
            // //移出购物车，所需7个参数，商铺id，食品分类id，食品id，食品规格id，食品名字，食品价格，食品规格
            // removeOutCart(category_id, item_id, food_id, name, price, specs){
            //     this.REDUCE_CART({shopid: state.shopId, category_id, item_id, food_id, name, price, specs});
            // },
            // /**
            //  * 初始化和shopCart变化时，重新获取购物车改变过的数据，赋值 categoryNum，totalPrice，cartFoodList，整个数据流是自上而下的形式，所有的购物车数据都交给vuex统一管理，包括购物车组件中自身的商品数量，使整个数据流更加清晰
            //  */
            ...
            //     this.ADD_CART({shopid: state.shopId, category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock});
            //     this.showChooseList();
            // },
            // //显示提示，无法减去商品
            // showReduceTip(){
        ...
        // watch: {
            ...
            watch(()=>state.shopCart,(value)=>{
                initCategoryNum()
            })
            //购物车列表发生变化，没有商铺时，隐藏
            watch(()=>state.cartFoodList,(value)=>{
                if(!value.length){
                    state.showCartList = false;
                }
            })
            //商品、评论切换状态
            watch(()=>changeShowType,(value)=>{
                if (value === 'rating') {
                    this.$nextTick(() => {
                        ...
                        state.ratingScroll.on('scroll', (pos) => {
                            if (Math.abs(Math.round(pos.y)) >=  Math.abs(Math.round(state.ratingScroll.maxScrollY))) {
            ...
            return {...toRefs(state),goback,afterEnter,beforeEnter,showMoveDotFun,showReduceTip,addSpecs,
chooseSpecs,showChooseList,hideLoading,loaderMoreRating,changeTgeIndex,
listenInCart,clearCart,toggleCartList,initCategoryNum,removeOutCart,addToCart,
showTitleDetail,chooseMenu,showActivitiesFun,listenScroll,getFoodListHeight,initData}
        }
    }
</script>
