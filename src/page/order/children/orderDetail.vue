            ...
            <section class="scroll_insert">
                ...
                <section class="food_list">
                    ...
                    <ul class="food_list_ul">
                        <li v-for="item in orderDetail.basket.group[0]" :key="item.name">
    ...
    export default {
      ...
      setup(){
            ...
            const state=reactive({
                ...
                orderData: null,
                imgBaseUrl,
                orderDetail:store.state.orderDetail,
                geohash:store.state.geohash,
                userInfo:store.state.userInfo
            })
            onMounted(()=>initData())
            watch(()=>userInfo,(val)=>{
                if (val && val.user_id)initData()
            })
            const initData=async()=>{
                if (state.userInfo && state.userInfo.user_id) {
                    state.orderData = await getOrderDetail(state.userInfo.user_id, state.orderDetail.unique_id);
