      ...
      setup(){
        ...
        onMounted(()=>{initData()})
            ...
            const initData=async()=>{
                if (store.state.userInfo && state.userInfo.user_id) {
                    let res = await getOrderList(store.state.userInfo.user_id, state.offset);
                    state.orderList = [...res];
                    hideLoading();
                }else{
                    hideLoading();
            ...
            const loaderMore=async()=>{
                if (state.preventRepeat) {
                    return
                }
                state.preventRepeat = true;
                state.showLoading = true;
                state.offset += 10;
                //获取信息
                let res = await getOrderList(state.userInfo.user_id, state.offset);
                state.orderList = [...state.orderList, ...res];
                state.hideLoading();
                if (res.length < 10) {
                    return
                }
                state.preventRepeat = false;
            },
            //显示详情页
            const showDetail=(item)=>{
                useMutation.SAVE_ORDER[0](item);
                state.preventRepeat = false;
        ...
        return {...toRefs(state),showDetail,loaderMore,initData}
        // methods: {
        //      ...mapMutations([
        //        'SAVE_ORDER'
        //     ])
        ...
        //         if (value && value.user_id && !state.orderList) {
        //             initData();
        //         }
        //     }
        // }
