    ...
    export default {
    	...
    	setup(){
            ...
            const initCity=async()=>{
                ...
                const cityname=await getCityName(state.cityid)
                state.cityname=cityname.data.name
            }
            const initData=()=>{
                //获取搜索历史记录
            ...
            const postpois=()=>{
                //输入值不为空时才发送信息
                if (state.inputVaule) {
                    searchplace(state.cityid, state.inputVaule).then(res => {
            ...
            return {
                ...toRefs(state),postpois,nextpage,clearAll}
            }
        }
        // mounted(){
