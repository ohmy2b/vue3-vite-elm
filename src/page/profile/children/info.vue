...
<script>
    import {reactive,toRefs,beforeUnmounted}from 'vue'
    ...
    export default {
        ...
        watch(() => store.state.userInfo, (val) => {
            ...
            const uploadAvatar=async()=>{
                ...
                if (state.userInfo) {
                    ...
                    try{
                        let response = await fetch('/eus/v1/users/' + state.userInfo.user_id + '/avatar', {
                              method: 'POST',
                              credentials: 'include',
                              body: data
                            })
                        ...
                        if (res.status == 1) {
                            state.userInfo.avatar = res.image_path;
                        }
                    }catch (error) {
                        state.showAlert = true;
        ...
        return {...toRefs(state),uploadAvatar,changePhone,exitlogin,outLogin,waitingThing}
    }
        // components: {
        //     headTop,
        //     alertTip,
