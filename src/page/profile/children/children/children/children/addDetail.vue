 <template>
  	<div class="rating_page">
        ...
        <section class="poisearch-container" v-show="adressList">
        	<ul>
        		<li v-for="(item, index) in adressList" @click="listClick(index)" :key="item.name">
        			<p>{{item.name}}</p>
        			<p>{{item.address}}</p>
        		</li>
        	</ul>
...
<script>
	...
	export default {
      ...
      setup(){
			...
			onMounted(()=>{state.inputAdress = state.addAddress ? state.addAddress : state.inputAdress})
            //搜索地址
            const inputThing=()=>{
            	searchNearby(state.inputAdress).then(res => {
            			state.adressList=res;
            			state.warning=true;
            		if(state.adressList.length > 0){
            			state.warning=false;
            			if(state.inputAdress == ''){
            				state.adressList=[];
            				state.warning=true;
            			}
            		}
            ...
            const listClick=(index)=>{
            	useMutation.SAVE_ADDDETAIL(state.adressList[index].name);
            	router.go(-1);
            }
			return {...toRefs(state),listClick,inputThing}
		}
        // created(){
        // 	state.inputAdress = state.addAddress ? state.addAddress : state.inputAdress
        
        // },
        // mounted(){
        // 	// getSearchAddress(state.inputAdress).then(res => {
        // 	// 		state.adressList=res;
        // 	// 		state.warning=true;
        // 	// 	if(state.adressList.length > 0){
        // 	// 		state.warning=false;
