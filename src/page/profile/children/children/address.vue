  	...
  	<div class="rating_page">
        ...
        <section class="address">
        	<ul class="addresslist">
        		<li v-for="(item,index) in removeAddress" :key="item.address">
        			<div>
        				<p>{{item.address}}</p>
...
<script>
    ...
    export default {
      ...
      components:{headTop,},
        mixins: [getImgPath],
      setup(){
			const store=useStore(),useAction=store._actions
			...
			watch(()=>store.state.userInfo,val=>{
				if(val&&val.user.id)initData()
			})
            //初始化信息
            const initData=()=>{
                if (state.userInfo && state.userInfo.user_id) {
                   useAction.saveAddress[0]();
                }
            }
            //编辑
            const editThing=()=>{
            	if(state.editText == '编辑'){
            		state.editText='完成';
            		state.deletesite=true;
            	}else{
            		state.editText='编辑';
            		state.deletesite=false;
            	}
            }
            //删除地址
            const deleteSite=async(index, item)=>{
                if (state.userInfo && state.userInfo.user_id) {
                    await deleteAddress(state.userInfo.user_id, item.id);
            	    state.removeAddress.splice(index, 1);
                }
            }
			return {...toRefs(state),deleteSite,editThing}
        },
        // mounted(){
        //     state.initData();
        // },
