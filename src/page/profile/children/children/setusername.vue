...
<script>
    ...
    export default {
      ...
      setup(){
            ...
            const inputThing=()=>{
                if(state.inputValue.length <5 || state.inputValue.length>24){
                    state.earn=false;
                    state.bordercolor=true;
                    state.opacityall=false;
                    return false;
                }else{
                    state.earn=true;
                    state.bordercolor=false;
                    state.opacityall=true;
                    return true;
                }
            }
            const resetName=()=>{
                let checkResult = state.inputThing();
                if (!checkResult) {
                  return;
                }
                useMutation.RETSET_NAME[0](state.inputValue);
                router.go(-1);
            }
            return {...toRefs(state),resetName,inputThing}
        },
        // computed:{
        //     ...mapState([
        //         'userInfo'
