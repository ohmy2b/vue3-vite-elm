...
<script>
    ...
    export default {
      ...
      setup(){
            ...
            couldPay=computed(()=> (/^\d{10}$/.test(state.cartNumber))&&(/^\d{6}$/.test(state.passWord)))
        // computed: {
        //     ...mapState([
        //         'userInfo', 
        //     ]),
        //     couldPay: function (){
        //     	return (/^\d{10}$/.test(state.cartNumber))&&(/^\d{6}$/.test(state.passWord)) ;
        //     },
        // },
        // components: {
        //     headTop,
        ...
        // },
            const confrimPay=async()=>{
            	if (state.couldPay) {
            		let res = await vipCart(state.userInfo.id, state.cartNumber, state.passWord);
            		if (res.message) {
            			state.showAlert = true;
            			state.alertText = res.message;
            		}else if(res.name){
            			state.showAlert = true;
            			state.alertText = res.name;
            		}
            	}
            }
            return {...toRefs(state),confrimPay,couldPay}
        }
    }
</script>
