...
<script>
    ...
    export default {
    	...
    	setup(props,content){
            ...
            const shopCart=computed(()=>Object.assign({},cartList[props.shopId]))
            ...
            const ADD_CART=store.mutation('ADD_CART')
            const REDUCE_CART=store.mutation('REDUCE_CART')
            const removeOutCart=(category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock)=>{
                if (foodNum > 0) {
                    REDUCE_CART({shopid: this.shopId, category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock});
                }
            }
            //加入购物车，计算按钮位置。
            const addToCart=(category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock, event)=>{
                ADD_CART({shopid: this.shopId, category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock});
                let elLeft = event.target.getBoundingClientRect().left;
                let elBottom = event.target.getBoundingClientRect().bottom;
                state.showMoveDot.push(true);
                content.emit('showMoveDot', state.showMoveDot, elLeft, elBottom);

            }
            //显示规格列表
            const showChooseList=(foodScroll)=>{
                content.emit('showChooseList', foodScroll)
            }
            //点击多规格商品的减按钮，弹出提示
            const showReduceTip=()=>{
                content.emit('showReduceTip')
            }
            return {...toRefs(state),removeOutCart,cartList,shopCart,addToCart,showChooseList,foodNum}
    }
}
        // computed: {
        //     // ...mapState([
